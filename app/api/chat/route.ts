...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === "sk-proj-your-openai-api-key-here") {
      // Use fallback system if no valid OpenAI key
      return await handleFallbackResponse(message, threadId)
    }

    // Create a system prompt for code generation
    const systemPrompt = `You are v0, an AI assistant that generates React/Next.js code. 
    When users ask for UI components or applications, respond with:
    1. A brief explanation of what you're building
    2. The complete, functional code
    
    Always use modern React patterns, TypeScript, and Tailwind CSS.
    Make components responsive and accessible.
    Use shadcn/ui components when appropriate.
    
    Format your response as:
    - Explanation text
    - Code blocks with file paths like: \`\`\`tsx file="component-name.tsx"
    
    Example response format:
    I'll create a modern login form component for you.
    ...
    // Fall back to template system on any error
    return await handleFallbackResponse(request.body.message, request.body.threadId)
  }
}

async function handleFallbackResponse(message: string, threadId: string) {
...
⚠️ Note: Using template system (OpenAI API not configured)`
